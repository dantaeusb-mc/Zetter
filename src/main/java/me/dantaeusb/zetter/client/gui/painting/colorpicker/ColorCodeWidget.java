...
public class ColorCodeWidget extends AbstractPaintingWidget implements Renderable {
  ...
  public final static int TEXTBOX_WIDTH = 82;
  public final static int TEXTBOX_HEIGHT = 16;

  public final static int TEXTBOX_TEXT_OFFSET = 10;
  ...
  public ColorCodeWidget(PaintingScreen parentScreen, int x, int y) {
    super(parentScreen, x, y, TEXTBOX_WIDTH, TEXTBOX_HEIGHT, Component.translatable("container.zetter.painting.color_code"));
  ...
  private void applyColor(String text) {
    ...
    try {
      ...
      //this.parentScreen.setPaletteColor(color);
  ...
  public boolean mouseClicked(double mouseX, double mouseY, int button) {
    if (isInRect(this.getX(), this.getY(), TEXTBOX_WIDTH, TEXTBOX_HEIGHT, (int) mouseX, (int) mouseY)) {
  ...
  protected void drawTextbox(GuiGraphics guiGraphics) {
    final int TEXTBOX_POSITION_U = 143;
    final int TEXTBOX_POSITION_V = 50;

    int textboxV = TEXTBOX_POSITION_V + (this.textField.isFocused() ? TEXTBOX_HEIGHT : 0);

    guiGraphics.blit(AbstractPaintingWidget.PAINTING_WIDGETS_TEXTURE_RESOURCE,  this.getX(), this.getY(), TEXTBOX_POSITION_U, textboxV, TEXTBOX_WIDTH, TEXTBOX_HEIGHT);
  }
}
