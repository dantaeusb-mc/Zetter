package me.dantaeusb.zetter.client.gui.painting.tool;

import me.dantaeusb.zetter.client.gui.PaintingScreen;
import me.dantaeusb.zetter.client.gui.painting.AbstractPaintingWidget;
import me.dantaeusb.zetter.client.gui.painting.base.TabsWidget;
import me.dantaeusb.zetter.painting.Tool;
import net.minecraft.network.chat.Component;

import java.util.Map;

public abstract class AbstractToolTabGroupWidget extends AbstractTabGroupWidget {
  public final static int X = 31;
  public final static int Y = 136;

  public final static int WIDTH = 164;
  public final static int HEIGHT = 120;

  private final Tool tool;

  public AbstractToolTabGroupWidget(PaintingScreen parentScreen, Component title, Tool tool, TabsWidget.Tab[] tabs, Map<String, AbstractPaintingWidget> tabWidgets) {
    super(parentScreen, X, Y, WIDTH, HEIGHT, title, tabs, tabWidgets);

    this.tool = tool;
  }

  public Tool getTool() {
    return this.tool;
  }
}
