...
public class CanvasRenderer implements AutoCloseable {
    ...
    public static CanvasRenderer getInstance() {
        if (instance == null) {
            instance = new CanvasRenderer(net.minecraft.client.Minecraft.getInstance().getTextureManager());
        }
        return instance;
    }
    ...
    public void update(long gameTime) {
        ...
        int partialTicks = (int) (gameTime - lastGameTime);
        if (partialTicks > 0) {
            lastGameTime = gameTime;
    ...
    protected void requestCanvasTexture(TextureRequest request) {
        ...
        net.neoforged.neoforge.network.PacketDistributor.sendToServer(requestSyncPacket);
    ...
    class Instance implements AutoCloseable {
        ...
        private Instance(String canvasCode, int width, int height, AbstractCanvasData.Resolution resolution) {
            ...
            this.canvasTexture = new DynamicTexture(width, height, false);
            this.canvasTexture.setFilter(false, false);
